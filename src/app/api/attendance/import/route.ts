    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            for (const row of uniqueRecords) {
                ...
                const departmentName = row.department?.trim() || 'Default Dept'
                const lineName = row.line?.trim() || 'Default Line'
                ...
                let dept = deptMap.get(departmentName)
                if (!dept) {
            ...
            // Skip daily data and ot parsing for overall view.
            // We ONLY synchronize employee master data.
        }, { timeout: 300000 })
