...
const dayResults_impl = (targetDate: Date, SHIFT_CONFIG: any, employees: any, fingerprintMaps: { prev: Map<string, string>, current: Map<string, string>, next: Map<string, string> }) => {
    ...
    for (const emp of employees) {
        const att = emp.attendances.find((a: any) => a.recordDate.getTime() === targetDate.getTime()) || (employees.length === 1 ? emp.attendances[0] : null)
    ...
    for (const emp of employees) {
        const att = emp.attendances.find((a: any) => a.recordDate.getTime() === targetDate.getTime()) || (employees.length === 1 ? emp.attendances[0] : null)
        const ot = emp.overtimes.find((o: any) => o.recordDate.getTime() === targetDate.getTime()) || (employees.length === 1 ? emp.overtimes[0] : null)
        if (!att && !ot) continue
        ...
        let inTime = todayInOut.inTime, outTime = null
    ...
    try {
        ...
        if (month) {
            ...
            emps = await prisma.employee.findMany({
                include: {
                    attendances: { where: { recordDate: { gte: start, lte: end } } },
                    overtimes: { where: { recordDate: { gte: start, lte: end } } },
        ...
        let emps: any[] = [];
        emps = await prisma.employee.findMany({ include: { attendances: { where: { recordDate: { gte: start, lte: end } } }, overtimes: { where: { recordDate: { gte: start, lte: end } } }, leaderHistories: { where: { startDate: { lte: end }, OR: [{ endDate: null }, { endDate: { gte: start } }] }, include: { leader: true } } } })
