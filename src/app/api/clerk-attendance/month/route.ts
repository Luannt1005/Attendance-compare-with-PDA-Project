...
export async function GET(req: Request) {
    try {
        ...
        if (monthParam?.startsWith('2026-04')) {
            console.log(`[DEBUG] Month param: "${monthParam}", LeaderId: "${leaderId}"`);
            const excelData = await getAprilExcelData(leaderId);
            console.log(`[DEBUG] Returning ${excelData.length} records for April 2026`);
